...
Qwen-Proxy 是一个将 `https://chat.qwen.ai` 和 `Qwen Code / Qwen Cli` 转换为 OpenAI 兼容 API 的代理服务。通过本项目，您只需要一个账户，即可以使用任何支持 OpenAI API 的客户端（如 ChatGPT-Next-Web、LobeChat 等）来调用 `https://chat.qwen.ai` 和 `Qwen Code / Qwen Cli`的各种模型。其中 `/cli` 端点下的模型由 `Qwen Code / Qwen Cli` 提供，原生 tools 参数支持。

**主要特性：**
- 兼容 OpenAI API 格式，对接常见客户端
- 兼容 Anthropic Messages API（`/v1/messages`），支持 Claude Code、Anthropic SDK 等客户端（近似兼容，非官方等价实现）
- 支持 Function Calling（OpenAI `tools`），通过提示词注入与流式状态机解析实现，非上游原生能力；含流式 `arguments` 增量分片与 `tool_choice=required` 强校验重试
- 支持多账户轮询，提高可用性
- 支持流式/非流式响应
- 支持多模态（图片识别、视频理解、图片/视频生成）
- 支持 OpenAI 风格资源端点：`/v1/images/generations`、`/v1/images/edits`、`/v1/videos`
- 支持智能搜索、深度思考等高级功能（通过模型名后缀 `-search`、`-thinking` 启用）
- 支持 CLI 端点，原生 tools 参数支持
- 提供 Web 管理界面，方便配置和监控
- 管理员密钥与下游 API Key 分离管理
- 请求日志监控页，支持按 API/Web 筛选
- 批量添加账号支持实时进度展示
- WAF 降载与自动重试机制，大 payload 自动压缩
...
`/v1/chat/completions` 支持 OpenAI Function Calling 协议。上游 Web 接口本身不具备原生 tools 能力，本服务通过提示词注入与流式状态机解析，使其行为与 OpenAI API 一致：

- 自动将 `tools[]` 压缩为 TS 风格签名注入提示词
- 流式输出按 OpenAI 规范分片：先发 `function.name + 空 arguments` 头块，随后多个 `arguments` 切片
- 历史消息中的 `assistant.tool_calls` 与 `role:"tool"` 自动折叠回链，`tool_call_id` 关联
- `tool_choice` 支持：`"auto"` / `"required"` / `{type:"function",function:{name:"..."}}` / `"none"`
- `tool_choice="required"` 或指定函数时，若首次未触发工具调用，自动追加强约束提示重试一次

> 注：大 payload 场景下 tools 说明可能被压缩或作为兜底删除，以降低 Qwen WAF 触发率。
...
CLI 端点使用 Qwen Code / Qwen Cli 的 OAuth 令牌访问，支持工具调用（Function Calling）。
...
| `coder-model` | Qwen 3.5 Plus（带思维链） |
